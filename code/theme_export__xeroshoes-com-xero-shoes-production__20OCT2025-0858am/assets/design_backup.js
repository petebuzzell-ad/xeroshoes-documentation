...
class storefrontFilters {
  constructor() {
    ...
    if (this.form) {
      ...
      this.applyfilterbtn.addEventListener('click', (evt) => {
        this.debouncedOnSubmit(evt);
      });
  ...
  */
  ...
  renderProductGrid(type, grid, banner,url) {
    ...
    // Trigger color filter auto-selection after HTML updates
    if (window.colorFilterAutoSelect && typeof window.colorFilterAutoSelect.autoSelectColorVariants === 'function') {
      setTimeout(() => {
        window.colorFilterAutoSelect.autoSelectColorVariants();
      }, 100);
    }
    ...
    if(typeof checkAllScrollOverflows == 'function') checkAllScrollOverflows();
